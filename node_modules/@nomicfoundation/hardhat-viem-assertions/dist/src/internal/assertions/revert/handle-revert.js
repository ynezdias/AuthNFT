...
export async function handleRevert(contractFn) {
    try {
        await contractFn;
    }
    catch (error) {
        ...
        if (isKnownErrorSelector(rawError.data) === false &&
            rawError.data !== "0x") {
            assert.fail(`The function was expected to revert with a non custom error, but it instead reverted with a custom error. ${rawError.message}`);
        }
        if (rawError.data === "0x") {
            return {
...
//# sourceMappingURL=handle-revert.js.map