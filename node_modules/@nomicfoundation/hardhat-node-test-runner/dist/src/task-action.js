...
import { markTestRunStart as initCoverage, markTestRunDone as reportCoverage, } from "hardhat/internal/coverage";
import { markTestRunStart as initGasStats, markTestRunDone as reportGasStats, } from "hardhat/internal/gas-analytics";
...
function isSubtestFailedError(error) {
...
async function getTestFiles(testFiles, config) {
    ...
    return getAllFilesMatching(config.paths.tests.nodejs, (f) => isJavascriptFile(f) || isTypescriptFile(f));
}
/**
 * Note that we are testing this manually for now as you can't run a node:test within a node:test
 */
const testWithHardhat = async ({ testFiles, only, grep, noCompile, testSummaryIndex }, hre) => {
    ...
    if (hre.globalOptions.gasStats === true) {
        const gasStats = new URL(import.meta.resolve("@nomicfoundation/hardhat-node-test-runner/gas-stats"));
    ...
    async function runTests() {
        ...
        const testOnlyMessage = "'only' and 'runOnly' require the --only command-line option.";
