...
export default async () => {
    const handlers = {
        registerFileForTestRunner: async (context, filePath, next) => {
            ...
            const inThisRunnersDirectory = absoluteFilePath.startsWith(context.config.paths.tests.nodejs);
            const notInOtherRunnersDirectory = allRunnersDirectories.every((runnersDirectory) => !absoluteFilePath.startsWith(runnersDirectory));
            const isSolidityFile = absoluteFilePath.endsWith(".sol");
            if (isSolidityFile === false &&
