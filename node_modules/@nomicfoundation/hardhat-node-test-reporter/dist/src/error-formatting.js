...
import { cleanupTestFailError, isTestFileExecutionFailureError, } from "./node-test-error-utils.js";
...
export function formatError(error) {
    if (isTestFileExecutionFailureError(error)) {
        return chalk.red(`Test file execution failed (exit code ${error.exitCode}).`);
    }
 ...
 */
export function formatSingleError(error, prefix = "", depth = 0) {
    const parsedStackLines = (error.stack?.split("\n") ?? []).map(parseStackLine);
    // Finds the first line which has a stack reference
    const firstLineWithReferenceIndex = parsedStackLines.findIndex((line) => line.reference !== undefined);
    // Finds the last line which does not have a node stack reference
    const lastNonNodeLineIndex = parsedStackLines.findLastIndex((line) => !isNodeLine(line));
    ...
    if (message.startsWith("AssertionError: The expression evaluated to a falsy value:")) {
    ...
    if (firstLineWithReferenceIndex === -1) {
        ...
        if (lastNonNodeLineIndex + 1 === firstLineWithReferenceIndex) {
            ...
            stackLines = parsedStackLines.slice(firstLineWithReferenceIndex, lastNonNodeLineIndex + 1);
        }
    }
    ...
    if (isAggregateError(error)) {
        ...
        const formattedErrors = error.errors
            .map((e) => indent(formatSingleError(e, "inner", depth), AGGREGATE_ERROR_INNER_ERROR_INDENT))
            .join("\n");
    ...
    if (error.cause instanceof Error) {
        ...
        const formattedCause = indent(formatSingleError(cause, "cause", depth + 1), ERROR_CAUSE_INDENT);
        return `${formattedError}\n${formattedCause}`;
    }
    return formattedError;
}
...
function isDiffableError(error) {
    return ("expected" in error && "actual" in error && error.expected !== undefined);
}
function getErrorDiff(error) {
...
export function formatStackLine({ line, reference }) {
 ...
 */
export function formatLocation(location, cwd = process.cwd(), sep = path.sep, windows = process.platform === "win32") {
...
function hasNodeContext({ context }) {
    ...
    const regex = new RegExp("^" + // Matches the beginning of the line
        "(?:" + // Opens a non-capturing group
