...
export function formatTestCancelledByParentFailure(failure) {
    return indent(chalk.grey(`${formatFailureIndex(failure.index)}) ${failure.testFail.name}`), nestingToIndentationLength(failure.testFail.nesting));
}
export function formatTestFailure(failure) {
    return indent(chalk.red(`${formatFailureIndex(failure.index)}) ${failure.testFail.name}`), nestingToIndentationLength(failure.testFail.nesting));
}
