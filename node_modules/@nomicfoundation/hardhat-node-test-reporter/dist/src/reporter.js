...
import { formatFailureReason, formatGlobalDiagnostics, formatUnusedDiagnostics, formatTestCancelledByParentFailure, formatTestContext, formatTestFailure, formatTestPass, formatSlowTestInfo, } from "./formatting.js";
import { annotatePR } from "./github-actions.js";
import { isCancelledByParentError, isSubtestFailedError, } from "./node-test-error-utils.js";
import { getTestRunOptions, isTopLevelFilePassEvent, } from "./node-test-utils.js";
 ...
 * @param source
 */
const customReporter = hardhatTestReporter(getTestRunOptions());
export default customReporter;
export function hardhatTestReporter(options, config = {
    testSummaryIndex: 0,
}) {
    return async function* (source) {
         ...
         */
        const stack = [];
        let lastPrintedIndex;
        const diagnostics = [];
        const preFormattedFailureReasons = [];
        let failureIndex = config.testSummaryIndex === 0 ? 1 : config.testSummaryIndex;
            ...
            switch (event.type) {
                ...
                case "test:fail": {
                    ...
                    if (event.data.details.type === "suite") {
                        ...
                        if (event.data.nesting === 0) {
                            ...
                            if (lastPrintedIndex !== topStackIndex) {
                                ...
                                yield formatTestContext([unprinted], "", "", event.data.skip === true);
                                yield "\n";
                    ...
                    if (lastPrintedIndex !== stack.length - 2) {
                        yield formatTestContext(stack.slice(lastPrintedIndex !== undefined ? lastPrintedIndex + 1 : 0, -1));
                        yield "\n";
        ...
        const { globalDiagnostics, unusedDiagnostics } = processGlobalDiagnostics(diagnostics);
