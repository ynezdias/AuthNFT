 ...
 */
export class EphemeralDeploymentLoader {
    _artifactResolver;
    ...
    async loadArtifact(artifactId) {
        ...
        assertIgnitionInvariant(saved !== undefined, `No stored artifact for ${futureId}`);
        switch (saved._kind) {
            case "artifact": {
            ...
            case "contractName": {
                ...
                assertIgnitionInvariant(fileArtifact !== undefined, `Unable to load artifact, underlying resolver returned undefined for ${saved.contractName}`);
                return fileArtifact;
            }
        }
    }
