 ...
 *  network interaction.
 */
export async function monitorOnchainInteraction(params) {
    const lastNetworkInteraction = params.exState.networkInteractions.at(-1);
    assertIgnitionInvariant(lastNetworkInteraction !== undefined, `No network interaction for ExecutionState ${params.exState.id} when trying to check its transactions`);
    assertIgnitionInvariant(lastNetworkInteraction.type === NetworkInteractionType.ONCHAIN_INTERACTION, `StaticCall found as last network interaction of ExecutionState ${params.exState.id} when trying to check its transactions`);
    assertIgnitionInvariant(lastNetworkInteraction.transactions.length > 0, `No transaction found in OnchainInteraction ${params.exState.id}/${lastNetworkInteraction.id} when trying to check its transactions`);
    ...
    if (transaction === undefined) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.DROPPED_TRANSACTION, {
            futureId: params.exState.id,
