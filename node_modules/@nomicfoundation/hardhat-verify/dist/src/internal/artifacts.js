 ...
 * build info is found.
 */
export async function getBuildInfoAndOutput(artifacts, contract) {
    ...
    const buildInfoOutputPath = await artifacts.getBuildInfoOutputPath(buildInfoId);
    if (buildInfoOutputPath === undefined) {
        return undefined;
    }
    const buildInfo = await readJsonFile(buildInfoPath);
    const buildInfoOutput = await readJsonFile(buildInfoOutputPath);
    return {
        buildInfo,
        buildInfoOutput,
    };
...
export async function getCompilerInput(solidity, root, sourceName, isNpmModule, buildProfileName) {
    const rootFilePath = isNpmModule
        ? `npm:${sourceName}`
        : path.join(root, sourceName);
    const getCompilationJobsResult = await solidity.getCompilationJobs([rootFilePath], {
        buildProfile: buildProfileName,
        quiet: true,
        force: true,
    });
    assertHardhatInvariant(!("reason" in getCompilationJobsResult), "getCompilationJobs should not error");
    const compilationJob = getCompilationJobsResult.compilationJobsPerFile;
    // TODO: should this be an error instead?
    assertHardhatInvariant(compilationJob instanceof Map && compilationJob.size === 1, "The compilation job for the contract source was not found.");
    const compilerInput = await compilationJob.get(rootFilePath)?.getSolcInput();
    // TODO: should this be an error instead?
    assertHardhatInvariant(compilerInput !== undefined, "The compiler input for the contract source was not found.");
