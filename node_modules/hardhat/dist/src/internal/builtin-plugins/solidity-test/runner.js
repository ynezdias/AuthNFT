 ...
 */
export function run(chainType, artifacts, testSuiteIds, configArgs, tracingConfig, sourceNameToUserSourceName, options) {
    const stream = new ReadableStream({
        async start(controller) {
            ...
            if (options?.timeout !== undefined) {
                timeout = setTimeout(() => {
                    controller.error(new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.RUNNER_TIMEOUT, {
            ...
            try {
                ...
                const solidityTestResult = await edrContext.runSolidityTests(hardhatChainTypeToEdrChainType(chainType), artifacts, testSuiteIds, configArgs, tracingConfig, (suiteResult) => {
                    controller.enqueue({
                        type: "suite:done",
                        data: suiteResult,
                    });
                ...
                ensureError(error);
                clearTimeout(timeout);
                controller.error(new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY_TESTS.UNHANDLED_EDR_ERROR_SOLIDITY_TESTS, {
                    error: error.message,
                }));
            }
        },
