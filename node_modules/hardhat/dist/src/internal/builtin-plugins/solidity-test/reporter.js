 ...
 * reporter's design.
 */
export async function* testReporter(source, sourceNameToUserSourceName, verbosity, colorizer = chalk) {
    ...
    const failuresByArtifactId = new Map();
    for (const { testResult, formattedArtifactId } of failures) {
        const artifactFailures = failuresByArtifactId.get(formattedArtifactId) ?? [];
