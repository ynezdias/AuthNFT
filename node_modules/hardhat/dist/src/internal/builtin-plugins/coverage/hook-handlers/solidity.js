        ...
        if (context.globalOptions.coverage && !isTestSource) {
            try {
                ...
                if (!satisfies(solcVersion, `<=${latestSupportedVersion}`)) {
                    console.log(`Solidity version ${solcVersion} is not yet supported for coverage instrumentation. Hardhat will try the latest supported version ${latestSupportedVersion} instead.`);
